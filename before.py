...
class PopupEmailAction(TemplateView):
    ...
    def handle(self, request):
        ...
        if request.method == "POST":
            if "send" in request.POST:
                messageform = self.messageform_class(
                    request.POST, message_info=messageform_info
                )
                # It's impossible for the form to be invalid, so assume valid
                messageform.is_valid()
                self.send_email(
                    request, wordwrap(messageform.cleaned_data["message"], 70)
                )
                return reroute_response(request)
            else:
                # cancel
...
class DbsConsentAlertLeaders(PopupEmailAction):
    ...
    def send_email(self, request, message):
        send_dbs_consent_alert_leaders_email(message, self.officer, self.camps)
        request.user.dbsactions_performed.create(
            officer=self.officer, action_type=DBSActionLog.ACTION_LEADER_ALERT_SENT
        )


dbs_consent_alert_leaders = staff_member_required(
    dbs_officer_required(DbsConsentAlertLeaders.as_view())
)
...
request_dbs_form_action = staff_member_required(
    dbs_officer_required(RequestDbsFormAction.as_view())
)
