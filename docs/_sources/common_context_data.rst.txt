...
* Over time, mixins defined like this quickly becomes tangled, due to that
  ...
  In contrast, all the parameters to a function are usually well defined, and
  you can usually have a very high level of confidence that it will work in all
  contexts. There is no ``self`` parameter which you can you use to sneak things
  in to a method. (See "shortcuts vs mixins" for a more treatment of this TODO)

* Mixins often tie you into inheritance trees for organising things. In reality,
  instead of tree want trees, you often want a mix-and-match approach to
  including this kind of data. Mixins do support that, at least in this case,
  but if you imagine that you'll have a neat hierarchy involving a
  ``HelpPageMixin`` that provides both context data and some other
  functionality, you'll quickly be disappointed when things don't turn out that
  simple, and you find yourself in a tangle.
